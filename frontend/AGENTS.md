...
- Do not call `fetch` directly; use the refresh orchestrator/client (lint allows direct fetch only in the refresh and data-access infrastructure, see `frontend/biome.json` and `frontend/biome-plugins/data-access-boundaries.grit`).
