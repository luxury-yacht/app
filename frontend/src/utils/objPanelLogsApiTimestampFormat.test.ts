...
describe('objPanelLogsApiTimestampFormat', () => {
  it('covers objPanelLogsApiTimestampFormat scenarios', async () => {
    // Scenario: accepts supported Day.js patterns used for Object Panel Logs Tab API timestamps
    expect(
    ...
    // Scenario: rejects unsupported tokens and malformed literals
    ...
    // Scenario: falls back to the default pattern when hydrating invalid values
    ...
    // Scenario: formats Kubernetes API timestamps in UTC using the provided pattern
    ...
    {
      // Scenario: formats Kubernetes API timestamps in the local timezone when enabled
