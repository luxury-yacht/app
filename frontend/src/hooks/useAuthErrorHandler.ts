...
export function useAuthErrorHandler(activeClusterId: string = ''): UseAuthErrorHandlerResult {
  ...
  useEffect(() => {
    ...
    const handleAuthFailed = (...args: unknown[]) => {
      ...
      // Create an error with retry capability for this specific cluster.
      // Note: Error notification is shown for any cluster failure.
      // The retryFn closure captures the clusterId so the UI can call it without parameters.
      errorHandler.handle(
        new Error(reason || 'Authentication failed'),
        {
          source: 'auth-manager',
          retryFn: () => handleRetry(clusterId),
        },
        `Authentication failed for ${clusterName || clusterId}. Please re-authenticate and click Retry.`
      );
    };
