...
it('reports the same resource failure independently for each cluster and deduplicates repeats', () => {
  ...
  events.listener!({ ...failure, clusterId: 'a' });
  events.listener!({ ...failure, clusterId: 'b' });
  events.listener!({ ...failure, clusterId: 'a' });
...
it('retains global stderr errors and suppresses errors already owned by the auth overlay', () => {
  events.listener!({ clusterId: '', source: 'stderr', message: 'helper failed' });
  events.listener!({ clusterId: '', source: 'stderr', message: 'helper failed' });
  events.listener!({ clusterId: 'a', message: 'Error loading SSO Token: expired' });
  events.listener!({ clusterId: 'a', message: 'no active clusters available' });
