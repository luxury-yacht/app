...
describe('resource metric selectors', () => {
  it('covers resource metric selectors scenarios', async () => {
    {
      // Scenario: selects a Pod row by full cluster/namespace/name identity
    ...
    {
      // Scenario: returns null when the referenced Pod row is absent or carries no metric data
    ...
    {
      // Scenario: selects workload metrics and parses ready pod counts from namespace-workloads rows
    ...
    {
      // Scenario: returns null when the referenced workload row is absent
      const payload: NamespaceWorkloadSnapshotPayload = makeNamespaceWorkloadSnapshotPayload({
        rows: [],
        metrics: { stale: true, successCount: 1, failureCount: 0 },
      });
    ...
    {
      // Scenario: selects Node metrics by cluster/name identity
    ...
    {
      // Scenario: returns null when the referenced Node row is absent
      const payload: ClusterNodeSnapshotPayload = makeClusterNodeSnapshotPayload({
