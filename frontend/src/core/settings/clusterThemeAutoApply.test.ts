...
describe('autoApplyClusterTheme', () => {
  it('covers autoApplyClusterTheme scenarios', async () => {
    {
      // Scenario: applies the matching theme and replays appearance overrides when current
    ...
    {
      // Scenario: does not apply a theme when the match result is stale
      let current = true;
    ...
    {
      // Scenario: does not hydrate or replay overrides when the apply step becomes stale
      let current = true;
