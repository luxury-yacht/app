...
describe('buildConnectivityPresentation', () => {
  it('covers buildConnectivityPresentation scenarios', async () => {
    for (const { overrides, status, summary } of [
      {
      ...
      // Scenarios: preserves precedence for $name
    ...
    {
      // Scenario: reports an immediate authentication retry without a countdown
    ...
    {
      // Scenario: shows "No cluster selected" when there is no lifecycle state (untracked/none selected)
      const presentation = buildConnectivityPresentation({
        clusterId: undefined,
    ...
    {
      // Scenario: shows restricted access — not perpetual loading — when namespace listing is permission-denied
      const presentation = buildConnectivityPresentation({
        clusterId: 'cluster-a',
        clusterName: 'alpha',
        lifecycleState: 'ready',
    ...
    {
      // Scenario: keeps ready copy stable while a connected cluster is refreshing
      const presentation = buildConnectivityPresentation({
        clusterId: 'cluster-a',
        clusterName: 'alpha',
        lifecycleState: 'ready',
    ...
    {
      // Scenario: presents a recovering cluster with a connectivity verdict as reconnecting
      const presentation = buildConnectivityPresentation({
        clusterId: 'cluster-a',
        clusterName: 'alpha',
        lifecycleState: 'ready',
    ...
    {
      // Scenario: presents a recovering cluster with an auth verdict as retrying authentication
      const presentation = buildConnectivityPresentation({
        clusterId: 'cluster-a',
        clusterName: 'alpha',
        lifecycleState: 'ready',
