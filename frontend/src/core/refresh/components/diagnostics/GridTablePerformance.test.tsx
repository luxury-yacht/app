...
describe('GridTablePerformance', () => {
  it('covers GridTablePerformance scenarios', async () => {
    {
      // Scenario: flags suspicious broad replacement and recompute signals
    ...
    {
      // Scenario: ignores one-off timing spikes until a metric has multiple samples
      const signals = buildTablePerformanceSignals(
        createRow({
    ...
    {
      // Scenario: does not flag render slow for a low average with a single high spike
      const signals = buildTablePerformanceSignals(
        createRow({
    ...
    {
      // Scenario: down-ranks broad replacement for live tables into an informational signal
      const signals = buildTablePerformanceSignals(
        createRow({
          mode: 'live',
          inputReferenceChanges: 9,
    ...
    {
      // Scenario: flags slow scroll-frame samples in diagnostics
      const signals = buildTablePerformanceSignals(
        createRow({
          scrollFrame: {
            windows: 1,
    ...
    {
      // Scenario: renders the most suspicious rows first with signal labels and churn ratios
    ...
    {
      // Scenario: renders explicit table modes for divergent view families
      const markup = renderToStaticMarkup(
        <GridTablePerformance
          onReset={() => undefined}
          summary="Rolling GridTable measurements for the instrumented large-data views."
    ...
    {
      // Scenario: builds a compact profiling overview for the current sample set
    ...
    {
      // Scenario: excludes informational live-churn notes from flagged-table counts
      const overview = buildTablePerformanceOverview([
        createRow({
          label: 'All Namespaces Workloads',
          mode: 'live',
    ...
    {
      // Scenario: identifies the dominant measured stage for a row
    ...
    {
      // Scenario: can narrow the view to flagged tables only
