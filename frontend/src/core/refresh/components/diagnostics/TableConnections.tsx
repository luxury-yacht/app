...
const ConnectionRow: React.FC<{ row: ConnectionsRow }> = ({ row }) => {
  ...
  return (
    <tr className="diagnostics-domain-row">
      <td className="diagnostics-domain-name" title={row.rowKey}>
        <span className="diagnostics-scope-indent">{row.leaf}</span>
...
export const ConnectionsTable: React.FC<ConnectionsTableProps> = ({ rows, callRows, summary }) => {
        ...
        <div className="diagnostics-section-title-group">
          <span className="diagnostics-section-subtitle">
            {filteredCallRows.length === callRows.length
              ...
              : `Reads that belong to no refresh domain • Showing: ${filteredCallRows.length}/${callRows.length}`}
          </span>
        </div>
