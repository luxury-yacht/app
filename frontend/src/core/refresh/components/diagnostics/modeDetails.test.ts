...
describe('resolveModeDetails', () => {
  it('covers resolveModeDetails scenarios', async () => {
    {
      // Scenario: labels a healthy stream as streaming
    ...
    {
      // Scenario: labels a poll-by-design domain as plain polling
      const details = resolveModeDetails({
    ...
    {
      // Scenario: labels a stream-covered domain polling on an unhealthy stream as a fallback
      const details = resolveModeDetails({
        domain: 'namespaces',
        streamMode: 'streaming',
        streamActive: true,
    ...
    {
      // Scenario: labels a drift-blocked scope distinctly — it will NOT self-heal
      const details = resolveModeDetails({
        domain: 'pods',
        streamMode: 'streaming',
        streamActive: false,
    ...
    // Scenario: keeps the stream-only and on-demand labels
    expect(
      resolveModeDetails({
