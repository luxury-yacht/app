...
describe('buildClusterDataRows', () => {
  it('gives a single-scope domain ONE row that names its own domain', () => {
  ...
  it('drops the cluster prefix the cluster header already states', () => {
    const rows = buildClusterDataRows({
      rows: [
        scopeRow({
          domain: 'catalog',
  ...
  it('does not treat the no-value placeholder as an error', () => {
  ...
  it('reports a domain that is not running as inactive, never as a fault', () => {
  ...
  it('counts exactly the rows whose badge says something is wrong', () => {
    ...
    const faults = rows.filter((row) => row.kind === 'scope' && row.health.tone === 'error').length;
    expect(rows[0].kind === 'cluster' && rows[0].issueSummary).toBe(`${faults} issues`);
    expect(faults).toBe(2);
  });

  it('still reports a real fault on a scope that is also idle', () => {
  ...
  it('folds status, health and staleness into one badge', () => {
    ...
    expect(pods?.kind === 'scope' && pods.health.tooltip).toContain('Health: healthy (delivering)');
  });

  it('states the transport and its poll in one Feed cell', () => {
    const rows = buildClusterDataRows({
      rows: [
  ...
  it('moves everything the eight columns omit into the row expander', () => {
    const rows = buildClusterDataRows({
      rows: [
        scopeRow({
          domain: 'pods',
  ...
  it('counts issues on the cluster header and leaves it empty when clean', () => {
