...
describe('scoped readiness, permission, and cluster auth transitions', () => {
  it('covers scoped readiness, permission, and cluster auth transitions scenarios', async () => {
    {
      // Scenario: coalesces deferred intent and resets permission only at a new epoch
      const first = {
    ...
    {
      // Scenario: makes duplicate auth lifecycle events no-ops
...
describe('ClusterRefreshRuntime', () => {
  it('covers ClusterRefreshRuntime scenarios', async () => {
    {
      // Scenario: keeps enabled scoped domain state behind runtime operations
      ...
      expect(
        runtime.setScopedDomainEnabled('pods', 'cluster-a|namespace:kube-system', true)
      ).toEqual({
        previous: undefined,
        changed: true,
      });
    ...
    {
      // Scenario: keeps live resource-stream table scopes enabled while typed query scopes run
      const runtime = new ClusterRefreshRuntime('cluster-a');
    ...
    {
      // Scenario: owns async streaming lifecycle bookkeeping
      const runtime = new ClusterRefreshRuntime('cluster-a');
      const cleanup = vi.fn();
    ...
    {
      // Scenario: tears down in-flight work and transient cluster state
      const runtime = new ClusterRefreshRuntime('cluster-a');
    ...
    {
      // Scenario: ignores stale settle and cancel events after a request is replaced
      const runtime = new ClusterRefreshRuntime('cluster-a');
      ...
      expect(
        runtime.settleInFlight('cluster-config', scope, firstRequest.requestId)
      ).toBeUndefined();
    ...
    {
      // Scenario: reference-counts scoped leases so concurrent holders share one enable
      const runtime = new ClusterRefreshRuntime('cluster-a');
    ...
    {
      // Scenario: tracks query and snapshot demand independently within one scoped lease count
      const runtime = new ClusterRefreshRuntime('cluster-a');
      const scope = 'cluster-a|';
    ...
    {
      // Scenario: skips polling for covered pods scopes while the stream is healthy and snapshots when it is not
    ...
    {
      // Scenario: never skips a stream-signal fetch — the doorbell IS the stream saying data changed
    ...
    {
      // Scenario: fetches a snapshot for a no-data scope even when the stream is healthy, but skips once it has data
      const runtime = new ClusterRefreshRuntime('cluster-a');
      const base = {
