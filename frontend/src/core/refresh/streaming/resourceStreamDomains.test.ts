...
describe('resource stream domain descriptors', () => {
  it('covers resource stream domain descriptors scenarios', async () => {
    // Scenario: covers every streamed resource domain exactly once
    ...
    // Scenario: declares scope and metrics behavior for every domain
    ...
    // Scenario: derives COMPLETE-only resource stream domains from the shared inventory
    ...
    // Scenario: normalizes scopes through descriptor scope kinds
    ...
    // Scenario: exposes domain guards from the descriptor table
    EXPECTED_DOMAINS.forEach((domain) => {
    ...
    // Scenario: keeps doorbell domains distinct from resource table domains
    ...
    {
      // Scenario: matches the backend-authored projection contract
    ...
    {
      // Scenario: normalizes scopes from the backend-authored executable examples
      ...
      >) {
        ...
        for (const example of cases.invalid) {
          expect(() => normalizeResourceScope(domain, example.scope)).toThrow(
            example.errorContains
          );
        }
      }
    }
