...
describe('normalizeResourceStreamProtocolMessage', () => {
  it('normalizes modern and legacy changed frames to the same canonical message', () => {
  ...
  it('rejects a modern signal clock that its domain does not declare', () => {
    expect(
      normalizeResourceStreamProtocolMessage({
        clusterId: 'cluster-a',
        domain: 'namespace-config',
...
describe('transitionResourceStreamProtocol', () => {
  it('uses ACK as the quiet-stream synchronization boundary', () => {
  ...
  it('resyncs when manager replacement sends COMPLETE before the initial RESET', () => {
    let state = initialResourceStreamProtocolState();
    ({ state } = transitionResourceStreamProtocol(state, {
      type: 'subscribe-sent',
      expectsReset: true,
  ...
  it('rejects replayed sequences and coalesces accepted changes behind one timer', () => {
    let state = initialResourceStreamProtocolState();
  ...
  it('turns permission denial into a terminal protocol state', () => {
  ...
  it('bounds coalescing and requests one forced resync on overflow', () => {
    let state = initialResourceStreamProtocolState();
  ...
  it('flushes observed clocks before clearing them for resync and then awaits ACK', () => {
    let state = initialResourceStreamProtocolState();
    let result = receive(state, {
      type: 'message-received',
  ...
  it('resumes from a sequence on reconnect and cancels coalescing when stopping', () => {
    let state = initialResourceStreamProtocolState();
    let result = receive(state, {
      type: 'message-received',
    ...
    expect(result.effects).toEqual([{ type: 'mark-resync-complete' }, { type: 'send-subscribe' }]);
  ...
  it('makes stopping terminal so late frames cannot affect a replacement owner', () => {
