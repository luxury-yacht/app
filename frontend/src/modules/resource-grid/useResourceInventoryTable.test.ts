...
describe('deriveResourceInventoryRenderState', () => {
  it('covers deriveResourceInventoryRenderState scenarios', async () => {
    {
      // Scenario: initializing with no rows shows the loading boundary, not empty
    ...
    {
      // Scenario: cold loading with no rows shows the loading boundary
    ...
    {
      // Scenario: ready with rows renders the table with no boundary or overlay
    ...
    {
      // Scenario: refreshing with rows shows the overlay, not the boundary
      const render = deriveResourceInventoryRenderState(
    ...
    {
      // Scenario: refreshing with no rows shows the boundary and never flashes empty
    ...
    {
      // Scenario: settled empty is the only state that renders the empty message
      const render = deriveResourceInventoryRenderState(
    ...
    {
      // Scenario: blocked is neither loading nor empty
      const render = deriveResourceInventoryRenderState(
    ...
    {
      // Scenario: error takes precedence over rows and loading
      const render = deriveResourceInventoryRenderState(
    ...
    {
      // Scenario: error with no rows before any settlement still renders through the boundary
    ...
    // Scenario: blocked takes precedence over an in-flight load but not over an error
    expect(
    ...
    {
      // Scenario: Local Partial surfaces the partial label and isPartial
      const render = deriveResourceInventoryRenderState(
        source({
          rows: [row('a')],
          loaded: true,
    ...
    {
      // Scenario: complete results never carry a partial label even if one is passed
      const render = deriveResourceInventoryRenderState(
        source({
          rows: [row('a')],
          loaded: true,
          completeness: 'complete',
          partialLabel: 'ignored',
        })
      );
