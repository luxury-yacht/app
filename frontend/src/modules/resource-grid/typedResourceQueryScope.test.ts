...
describe('typedResourceQueryScope', () => {
  it('covers typedResourceQueryScope scenarios', async () => {
    {
      // Scenario: builds a stable all-namespaces resource query scope
    ...
    {
      // Scenario: serializes opaque facet selections as repeated query values
    ...
    {
      // Scenario: serializes Event and Application triage facet selections as provider-owned query keys
      const scope = buildTypedResourceQueryScope('cluster-a', {
    ...
    {
      // Scenario: sends includeMetadata in the scope when metadata search is enabled
      const scope = buildTypedResourceQueryScope('cluster-a', {
    ...
    {
      // Scenario: changes the query identity when metadata search toggles
    ...
    {
      // Scenario: changes the query identity when a provider-owned facet changes
      const base = {
    ...
    {
      // Scenario: builds the same query identity for equivalent unordered filters
    ...
    {
      // Scenario: includes cluster, domain, and page limit in lifecycle identity
      const base = {
        enabled: true,
    ...
    // Scenario: projects advertised status and node facets into query controls
    expect(
      filterOptionsFromTypedPayload({
        facetValues: [
          {
    ...
    // Scenario: projects a new provider facet without shared key-specific metadata
    expect(
      filterOptionsFromTypedPayload({
        facetValues: [
          {
    ...
    {
      // Scenario: projects searchable Event triage facets and approximate Application triage facets
