...
describe('backendQuerySource', () => {
  it('covers backendQuerySource scenarios', async () => {
    {
      // Scenario: treats a disabled query as blocked with no pagination
    ...
    {
      // Scenario: defaults completeness to complete and carries no partial label
    ...
    {
      // Scenario: surfaces a degraded (partial) envelope with its label
      const source = backendQuerySource(
        input({
    ...
    {
      // Scenario: a warm reload that momentarily returns nothing renders as loading, not empty
    ...
    {
      // Scenario: a settled-empty query renders the empty state
      const render = deriveResourceInventoryRenderState(
    ...
    {
      // Scenario: passes the query error through to the render state
      const render = deriveResourceInventoryRenderState(
