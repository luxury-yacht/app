...
describe('boundedRowsSource', () => {
  it('covers boundedRowsSource scenarios', async () => {
    {
      // Scenario: treats Local Complete as the complete matching set
    ...
    // Scenario: defaults to Local Complete when no mode is given
    expect(boundedRowsSource({ rows }).completeness).toBe('complete');

    {
      // Scenario: treats Local Partial as a capped view carrying its window label
    ...
    {
      // Scenario: renders an empty bounded array as settled empty, not loading
    ...
    {
      // Scenario: surfaces a still-filling bounded array as loading until loaded
    ...
    // Scenario: propagates blocked and error states
...
describe('boundedRowsSource bound-proof', () => {
  it('covers boundedRowsSource bound-proof scenarios', async () => {
    {
      // Scenario: renders capped/windowed bounded data as partial, never as a complete table
      const render = deriveResourceInventoryRenderState(
        boundedRowsSource({
          rows,
          loaded: true,
    ...
    {
      // Scenario: only reports complete (no partial banner) for genuinely bounded sets
