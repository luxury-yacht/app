...
describe('structuralShareResourceRows', () => {
  it('covers structuralShareResourceRows scenarios', async () => {
    {
      // Scenario: reuses the previous array, row, and ref when every value is unchanged
    ...
    {
      // Scenario: keeps the ref but allocates the incoming row when a scalar changes
      const previous = [row('api')];
    ...
    for (const [_label, overrides] of [
      ['map', { labels: { app: 'api', tier: 'backend' } }],
      ['array', { conditions: [{ type: 'Ready', status: 'False' }] }],
    ] satisfies Array<[string, Parameters<typeof row>[1]]>) {
      // Scenarios: does not mask a changed nested %s value
      const previous = [row('api')];
    ...
    {
      // Scenario: allocates a new ref and row when canonical identity changes
      const previous = [row('api')];
    ...
    {
      // Scenario: reuses matching rows across reordering without reusing the old array
    ...
    {
      // Scenario: reuses surviving rows across insertion and deletion
      const api = row('api');
    ...
    {
      // Scenario: does not reuse ambiguous duplicate canonical keys
    ...
    {
      // Scenario: reuses refs but skips recursive row comparison in ref-only mode
      const previous = [row('api')];
      const incoming = [row('api')];
