...
describe('queryBackedTableState', () => {
  it('covers queryBackedTableState scenarios', async () => {
    {
      // Scenario: excludes selected query facets from both controls and backend query state
    ...
    // Scenario: keeps explicit namespace options instead of collapsing to filtered query facets
    expect(queryBackedFacetFilterOptions(['team-a', 'team-b'], ['team-a'])).toEqual([
      'team-a',
      'team-b',
    ]);
    // Scenario: keeps explicit namespace metadata when it includes namespaces beyond the loaded row fallback
    expect(queryBackedFacetFilterOptions(['team-a', 'team-b'], ['team-a'], ['team-a'])).toEqual([
      'team-a',
      'team-b',
    ]);
    // Scenario: uses query facet namespaces when explicit options are only the loaded row fallback
    ...
    {
      // Scenario: preserves an explicit selection containing every namespace
    ...
    // Scenario: omits an explicit all-namespace selection from the backend query
    expect(
      normalizeQueryBackedNamespaceQueryFilters(
        {
          ...DEFAULT_GRID_TABLE_FILTER_STATE,
    ...
    // Scenario: drops persisted all-namespace sentinel values before building query scope state
    expect(
      removeQueryBackedNamespaceFilterSentinels({
        ...DEFAULT_GRID_TABLE_FILTER_STATE,
    ...
    {
      // Scenario: preserves real namespace subsets
      const filters: GridTableFilterState = {
        ...DEFAULT_GRID_TABLE_FILTER_STATE,
