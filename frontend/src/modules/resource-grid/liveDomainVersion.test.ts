...
describe('liveDomainVersion', () => {
  it('covers liveDomainVersion scenarios', async () => {
    {
      // Scenario: is stable when only the refresh timestamp changes (same data)
    ...
    {
      // Scenario: is stable across lastAutoRefresh / lastManualRefresh churn
    ...
    // Scenario: changes when a doorbell clock changes
    ...
    {
      // Scenario: is stable when a payload apply rewrites the folded sourceVersion (no echo)
    ...
    {
      // Scenario: ignores legacy version/checksum/etag/streamRevision components
    ...
    // Scenario: keeps the folded token for domains without doorbell clocks
