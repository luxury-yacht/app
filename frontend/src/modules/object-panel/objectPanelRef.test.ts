...
// The PodsTab leases a `pods` window per panel object (metrics arrive joined on
// the same domain's rows at serve); closing the panel must evict it like the
// other panel scopes.
describe('getObjectPanelScopeEvictions pods window', () => {
  it('covers getObjectPanelScopeEvictions pods window scenarios', async () => {
    {
      // Scenario: evicts the workload pods window for a workload panel
    ...
    {
      // Scenario: evicts the node pods window for a node panel
      const evictions = getObjectPanelScopeEvictions({
        clusterId: 'cluster-a',
        group: '',
        kind: 'Node',
    ...
    {
      // Scenario: does not evict a pods window for kinds without a pods tab
      const evictions = getObjectPanelScopeEvictions({
        clusterId: 'cluster-a',
        kind: 'ConfigMap',
        version: 'v1',
...
describe('getObjectPanelScopes', () => {
  it('covers getObjectPanelScopes scenarios', async () => {
    {
      // Scenario: normalises kind casing and builds scopes for standard resources
    ...
    {
      // Scenario: returns null refresh scopes when cluster identity is missing
      const result = getObjectPanelScopes({
        group: '',
        kind: 'Pod',
        name: 'api',
    ...
    {
      // Scenario: returns null refresh scopes when GVK identity is missing
      const result = getObjectPanelScopes({
        clusterId: 'cluster-1',
        kind: 'Deployment',
        name: 'api',
    ...
    {
      // Scenario: returns null refresh scopes when a non-core builtin has an empty group segment
      const result = getObjectPanelScopes({
        clusterId: 'cluster-1',
        group: '',
        kind: 'Deployment',
    ...
    {
      // Scenario: falls back to cluster scope when namespace is empty
      const result = getObjectPanelScopes(
        {
          clusterId: 'cluster-1',
          kind: 'HelmRelease',
    ...
    {
      // Scenario: keeps real HelmRelease custom resources on their supplied GVK
      const result = getObjectPanelScopes({
        kind: 'HelmRelease',
        name: 'flux-app',
        namespace: 'apps',
    ...
    {
      // Scenario: marks event resources with event-specific flag
      const result = getObjectPanelScopes({
        clusterId: 'cluster-1',
        group: '',
        kind: 'Event',
    ...
    {
      // Scenario: emits the GVK scope form when PanelObjectData carries group and version
    ...
    {
      // Scenario: emits the GVK scope form for core resources with an empty group
    ...
    {
      // Scenario: builds eventsScope from the original-case kind so a single source of truth feeds both consumers
    ...
    {
      // Scenario: threads group/version into eventsScope when PanelObjectData carries them
      const result = getObjectPanelScopes({
        kind: 'DBInstance',
        name: 'orders',
        namespace: 'team-a',
    ...
    // Scenario: returns null eventsScope when objectData is incomplete
    ...
    {
      // Scenario: builds containerLogsScope using the lowercased kind so ObjectPanelContent and LogViewer agree
    ...
    {
      // Scenario: threads group/version into containerLogsScope when PanelObjectData carries them
      const result = getObjectPanelScopes({
        kind: 'Deployment',
        name: 'api',
        namespace: 'team-a',
    ...
    {
      // Scenario: keeps colliding kinds distinct in containerLogsScope by threading group/version
    ...
    // Scenario: returns null containerLogsScope when objectData is incomplete
    ...
    {
      // Scenario: builds mapScope using the original-case kind so it matches the backend object-map parser
      const result = getObjectPanelScopes({
        kind: 'Deployment',
        name: 'api',
        namespace: 'team-a',
    ...
    // Scenario: returns null mapScope when objectData is incomplete
    ...
    // Scenario: supports policy resources as object-map seeds
    ...
    // Scenario: supports network policies as object-map seeds
    ...
    // Scenario: supports Gateway API resources as object-map seeds
    for (const kind of [
      'GatewayClass',
      'Gateway',
      'HTTPRoute',
