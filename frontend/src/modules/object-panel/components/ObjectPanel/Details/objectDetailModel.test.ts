...
describe('objectDetailModel', () => {
  it('covers objectDetailModel scenarios', async () => {
    {
      // Scenario: exposes the active detail payload and cronjob suspend
    ...
    {
      // Scenario: selects workload containers and active pod names
      const deploy = {
        desiredReplicas: 3,
    ...
    {
      // Scenario: selects data sections for configmaps and secrets
    ...
    {
      // Scenario: detects port-forward availability for pod-like resources and services
    ...
    // Scenario: selects desired replicas only for scalable workload kinds
    expect(
    ...
    {
      // Scenario: excludes Jobs from the container section even though JobDetails carries containers
    ...
    {
      // Scenario: does not treat Ingress rules as RBAC roleRules
    ...
    {
      // Scenario: reports zero desired scale replicas for HPA (not scalable via the scale action)
    ...
    {
      // Scenario: does not derive activePodNames for a Node even though NodeDetails carries pods
    ...
    {
      // Scenario: selects Namespace finalization diagnostics from the typed detail DTO
      const detail = {
