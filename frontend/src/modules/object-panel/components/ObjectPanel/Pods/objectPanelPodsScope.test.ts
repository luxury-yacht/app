...
describe('buildObjectPanelPodsScope', () => {
  it('covers buildObjectPanelPodsScope scenarios', async () => {
    // Scenario: returns null when the object name is missing
    expect(
      buildObjectPanelPodsScope({ kind: 'Deployment', namespace: 'team-a' }, 'deployment')
    ).toBeNull();
    // Scenario: returns null when the kind is missing
    expect(buildObjectPanelPodsScope({ name: 'api', namespace: 'team-a' }, null)).toBeNull();
    // Scenario: builds a node scope for Node panels
    expect(buildObjectPanelPodsScope({ kind: 'Node', name: 'worker-a' }, 'node')).toBe(
      'node:worker-a'
    );

    {
      // Scenario: builds a workload scope from the object GVK
    ...
    // Scenario: returns null when a workload object omits group/version/kind segments
    expect(
      buildObjectPanelPodsScope({ name: 'api', namespace: 'team-a' }, 'statefulset')
    ).toBeNull();
    // Scenario: returns null when a workload object has kind and version but omits group
    expect(
    ...
    // Scenario: uses the batch group for jobs
    expect(
      buildObjectPanelPodsScope(
    ...
    // Scenario: returns null for a workload without a namespace
    expect(buildObjectPanelPodsScope({ kind: 'Deployment', name: 'api' }, 'deployment')).toBeNull();
    // Scenario: returns null for unsupported kinds
    expect(
      buildObjectPanelPodsScope(
    ...
    // Scenario: returns null when the object data is null
