...
describe('yamlFieldPolicy', () => {
  it('covers yamlFieldPolicy scenarios', async () => {
    // Scenario: loads the backend-owned contract
    expect(
    ...
    {
      // Scenario: strips ignored server-owned fields for semantic comparison and prunes empty maps
...
`;
      ...
      expect(sanitizeYamlForSemanticCompare(stored)).toBe(
    ...
    {
      // Scenario: keeps meaningful user-controlled semantic differences
...
`;
    ...
    {
      // Scenario: resolves protected ranges for scalars, annotation keys, managedFields, flow maps, and status
...
`;
    ...
    {
      // Scenario: keeps adjacent scalar protected fields on their own lines
      const yaml = `apiVersion: apps/v1
kind: Deployment
metadata:
  name: demo
`;
      ...
      const ranges = resolveProtectedYamlRanges(yaml);
      const apiVersionRange = ranges.find((range) =>
        range.blockedMessage?.startsWith('apiVersion ')
      );
    ...
    {
      // Scenario: does not create managedFields ranges when managedFields are hidden or absent
      const yaml = `apiVersion: apps/v1
kind: Deployment
metadata:
  labels:
...
`;
    ...
    // Scenario: returns no protected ranges for invalid YAML
    expect(resolveProtectedYamlRanges('metadata:\n  name: [')).toEqual([]);

    {
      // Scenario: includes adjacent comments with protected fields
