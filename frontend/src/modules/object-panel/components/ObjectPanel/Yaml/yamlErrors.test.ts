...
describe('parseObjectYamlError', () => {
  it('covers parseObjectYamlError scenarios', async () => {
    // Scenario: returns null for non-matching errors
    expect(parseObjectYamlError(new Error('plain error'))).toBeNull();

    {
      // Scenario: parses structured payloads from error strings
    ...
    {
      // Scenario: handles invalid JSON gracefully
