...
describe('logFilterSelection', () => {
  it('covers logFilterSelection scenarios', async () => {
    {
      // Scenario: keeps pods open-ended when narrowing only the container group
    ...
    {
      // Scenario: keeps explicit pod narrowing while leaving containers open-ended
      const selection = logFilterSelectionFromDropdownValues(
    ...
    {
      // Scenario: preserves an explicitly empty pod group while pruning unavailable options
      const selection = logFilterSelectionFromDropdownValues(
    ...
    {
      // Scenario: preserves an explicitly empty container group while pruning unavailable options
    ...
    {
      // Scenario: replaces an empty pod group when selecting only one pod
      const selection = logFilterSelectionFromDropdownValues(
        ['container:app', 'container:sidecar'],
        options
      );
