...
describe('mergeAnchoredLogEntries', () => {
  it('covers mergeAnchoredLogEntries scenarios', async () => {
    {
      // Scenario: retains trimmed head entries and appends the new tail
    ...
    {
      // Scenario: recognizes fallback snapshot overlap when sequence keys are regenerated
    ...
    {
      // Scenario: keeps the anchored snapshot when an incoming snapshot is already represented
