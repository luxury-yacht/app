...
describe('NsViewWorkloads helpers', () => {
  it('covers NsViewWorkloads helpers scenarios', async () => {
    // Scenario: normalizes workload kinds with canonical casing
    ...
    // Scenario: clamps replica values within expected bounds
    expect(clampReplicas(-5)).toBe(0);
    expect(clampReplicas(42)).toBe(42);
    expect(clampReplicas(20000)).toBe(9999);
    // Scenario: parses replica counts from ready strings safely
    ...
    {
      // Scenario: builds and parses workload keys with namespace context
    ...
    {
      // Scenario: appends workload tokens for search filtering
      const tokens: string[] = [];
