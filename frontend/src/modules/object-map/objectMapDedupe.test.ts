...
describe('dedupeServiceEdges', () => {
  it('covers dedupeServiceEdges scenarios', async () => {
    {
      // Scenario: drops the direct Service→Pod selector edge when an endpoint chain to the same Pod exists
    ...
    {
      // Scenario: keeps the selector edge when the Pod is not in the endpoint chain (divergence case)
    ...
    {
      // Scenario: drops the Service→EndpointSlice owner edge when a parallel endpoint edge exists
      const nodes: ObjectMapNode[] = [
        node('svc', 'Service', 'web'),
        node('es', 'EndpointSlice', 'web-xyz'),
      ];
    ...
    {
      // Scenario: keeps the Service→EndpointSlice owner edge when no endpoint edge exists
    ...
    {
      // Scenario: does not affect non-Service relationships
    ...
    {
      // Scenario: handles a Service with multiple EndpointSlices targeting overlapping Pods
      const nodes: ObjectMapNode[] = [
        node('svc', 'Service', 'web'),
