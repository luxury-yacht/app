...
describe('filterByDirectionalReachability', () => {
  it('covers filterByDirectionalReachability scenarios', async () => {
    {
      // Scenario: keeps the backward chain from a Node seed but drops the Pods' forward dependencies
    ...
    {
      // Scenario: keeps both forward and backward chains independently from a Pod seed
    ...
    {
      // Scenario: drops edges whose endpoints are filtered out
      const nodes: ObjectMapNode[] = [
        node('seed', 'Pod', 'p'),
    ...
    {
      // Scenario: returns the original payload if the seed is not in the node list
    ...
    {
      // Scenario: returns just the seed when it has no edges
      const nodes: ObjectMapNode[] = [
