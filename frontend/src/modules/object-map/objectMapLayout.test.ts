...
describe('computeObjectMapLayout', () => {
  it('covers computeObjectMapLayout scenarios', async () => {
    {
      // Scenario: returns empty result when there are no nodes
    ...
    {
      // Scenario: places each generation in its own column when ownership flows from the seed
    ...
    {
      // Scenario: places ancestors in negative columns once the seed is shifted to 0
      const nodes: ObjectMapNode[] = [
    ...
    {
      // Scenario: pulls graph sources rightward to sit adjacent to their successors
    ...
    {
      // Scenario: eliminates same-column edges in acyclic graphs by extending edges to the longest path
    ...
    {
      // Scenario: falls back to deterministic kind/name order when no edges constrain barycenters
      const nodes: ObjectMapNode[] = [
        node('seed', 0, 'Deployment', 'web'),
    ...
    {
      // Scenario: orders columns by barycenter so connected nodes line up across depths
      const nodes: ObjectMapNode[] = [
        node('seed', 0, 'Deployment', 'web'),
    ...
    {
      // Scenario: routes cross-column edges from source-right to target-left as a cubic bezier
      const nodes: ObjectMapNode[] = [
        node('seed', 0, 'Deployment', 'web'),
        node('rs', 1, 'ReplicaSet', 'web-1'),
      ];
    ...
    {
      // Scenario: falls back to backend depth and arc-routes a same-column edge when an edge cycle traps both endpoints
    ...
    {
      // Scenario: groups same-kind nodes together within a column with extra padding between kinds
      ...
      const expectedKindGap =
        OBJECT_MAP_NODE_HEIGHT + OBJECT_MAP_ROW_GAP + OBJECT_MAP_KIND_GROUP_GAP;
      expect(rs.y - cm.y).toBe(expectedKindGap);
      expect(svc.y - rs.y).toBe(expectedKindGap);
    }

    {
      // Scenario: splits overloaded columns into horizontal lanes
    ...
    {
      // Scenario: keeps same-kind groups together when overloaded columns split into lanes
    ...
    {
      // Scenario: shifts later logical columns after split lanes so they do not overlap
      const manyPods = Array.from({ length: OBJECT_MAP_MAX_NODES_PER_LANE + 1 }, (_, index) =>
        node(`pod-${index}`, 1, 'Pod', `pod-${String(index).padStart(2, '0')}`)
      );
      const nodes: ObjectMapNode[] = [
    ...
    {
      // Scenario: drops edges that reference unknown nodes
    ...
    {
      // Scenario: reports bounds that wrap every node
      const nodes: ObjectMapNode[] = [
        node('seed', 0, 'Deployment', 'web'),
        node('rs', 1, 'ReplicaSet', 'web-1'),
        node('a', 2, 'Pod', 'a'),
