...
describe('object map legend drag helpers', () => {
  it('covers object map legend drag helpers scenarios', async () => {
    {
      // Scenario: clamps legend position inside the canvas padding
    ...
    {
      // Scenario: detects interactive legend targets
    ...
    {
      // Scenario: starts drag from the current clamped legend position
    ...
    {
      // Scenario: does not start drag from secondary buttons or interactive controls
      const button = document.createElement('button');
      const canvasRect = rect({ width: 300, height: 200 });
      const legendRect = rect({ width: 80, height: 60 });
    ...
    {
      // Scenario: moves only the active pointer and clamps the result
      const canvasRect = rect({ width: 300, height: 200 });
      const legendRect = rect({ width: 80, height: 60 });
      const drag = {
        pointerId: 4,
    ...
    {
      // Scenario: ends drag only for the active pointer
      const drag = {
        pointerId: 4,
        originClientX: 20,
        originClientY: 30,
