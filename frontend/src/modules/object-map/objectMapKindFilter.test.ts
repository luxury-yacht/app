...
describe('contractObjectMapKindFilter', () => {
  it('covers contractObjectMapKindFilter scenarios', async () => {
    {
      // Scenario: contracts directed paths through nodes hidden by the Kinds filter
      ...
      expect(
        result.edges[0].filteredPath?.nodes.map((entry) => [entry.id, entry.filtered])
      ).toEqual([
        ['service', false],
        ['endpoint-slice', true],
        ['pod', false],
    ...
    {
      // Scenario: merges multiple hidden paths and keeps the shortest tooltip path
      const result = contractObjectMapKindFilter(
        [
          node('service', 'Service', 'web'),
