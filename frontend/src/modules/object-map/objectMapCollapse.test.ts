...
describe('computeCollapseInfo', () => {
  it('covers computeCollapseInfo scenarios', async () => {
    {
      // Scenario: shows all ReplicaSets that still own Pods
    ...
    {
      // Scenario: hides zero-Pod ReplicaSets when collapsed
      const nodes: ObjectMapNode[] = [
        node('dep', 'Deployment', 'web'),
        node('rs-current', 'ReplicaSet', 'web-aaa'),
        node('rs-old', 'ReplicaSet', 'web-bbb'),
    ...
    {
      // Scenario: reveals all RSs when the Deployment is expanded
      const nodes: ObjectMapNode[] = [
        node('dep', 'Deployment', 'web'),
        node('rs-current', 'ReplicaSet', 'web-aaa'),
        node('rs-old', 'ReplicaSet', 'web-bbb'),
    ...
    {
      // Scenario: anchors the collapse badge on the RS with more owned Pods
      const nodes: ObjectMapNode[] = [
        node('dep', 'Deployment', 'web'),
    ...
    {
      // Scenario: falls back to lexicographically larger name when pod counts tie
    ...
    {
      // Scenario: keeps the seed's RS visible even when it isn't the current one
    ...
    {
      // Scenario: emits no group when a Deployment has only one RS
      const nodes: ObjectMapNode[] = [
        node('dep', 'Deployment', 'web'),
    ...
    {
      // Scenario: leaves standalone ReplicaSets (no Deployment owner) visible and ungrouped
      const nodes: ObjectMapNode[] = [
