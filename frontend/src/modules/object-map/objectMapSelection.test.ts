...
describe('computeObjectMapSelectionState', () => {
  it('covers computeObjectMapSelectionState scenarios', async () => {
    {
      // Scenario: walks forward and backward from the active node
    ...
    {
      // Scenario: returns an empty selection when no node is active
...
describe('isObjectMapEdgeDimmedBySelection', () => {
  ...
  it('covers isObjectMapEdgeDimmedBySelection scenarios', async () => {
    {
      // Scenario: never dims when no node is selected
    ...
    {
      // Scenario: dims only edges unrelated to the active selection
