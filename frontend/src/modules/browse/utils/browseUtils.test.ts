...
describe('reconcileByUID', () => {
  it('covers reconcileByUID scenarios', async () => {
    {
      // Scenario: reuses existing item references when resource versions are unchanged
      const current = [
    ...
    {
      // Scenario: returns a new list when an item resource version changed
    ...
    {
      // Scenario: does not hide derived action-fact changes when the object resource version is unchanged
      const current = [
        makeItem({
    ...
    {
      // Scenario: reflects deletions in full replacement snapshots
      const current = [
