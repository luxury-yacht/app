...
describe('browseCatalogData', () => {
  it('covers browseCatalogData scenarios', async () => {
    {
      // Scenario: plans base, metadata, and page scopes for pinned namespace Browse
    ...
    {
      // Scenario: separates user namespace scope from backend namespace expansion
      const plan = buildBrowseCatalogPlan({
        clusterId: 'cluster-1',
        clusterScopedOnly: false,
        pinnedNamespaces: [],
    ...
    {
      // Scenario: carries the cluster Browse boundary separately from active filters
      const plan = buildBrowseCatalogPlan({
        clusterId: 'cluster-1',
    ...
    {
      // Scenario: includes backend sort in catalog scopes when the sort is not the default
      const plan = buildBrowseCatalogPlan({
        clusterId: 'cluster-1',
        clusterScopedOnly: false,
        pinnedNamespaces: ['default'],
    ...
    // Scenario: rejects stale pinned-namespace snapshots
    expect(
      acceptsCatalogSnapshotScope(
    ...
    {
      // Scenario: applies baseline snapshots as full replacements
    ...
    {
      // Scenario: applies page snapshots as current-window replacement pagination
      const first = makeItem({ ref: { uid: 'pod-a', name: 'pod-a' } });
      const second = makeItem({ ref: { uid: 'pod-b', name: 'pod-b' } });
      const existing = applyCatalogBaseline(
        emptyBrowseCatalogCollection(),
    ...
    {
      // Scenario: derives scope-aware filter options from catalog metadata
    ...
    {
      // Scenario: derives reason-bearing degraded copy from catalog query metadata
      const payload = makePayload({
        totalIsExact: false,
        facetsExact: false,
        issues: [
