...
describe('createInitialTabGroupState', () => {
  it('covers createInitialTabGroupState scenarios', async () => {
    {
      // Scenario: returns empty state with no tabs
    ...
    {
      // Scenario: returns a new object each time
...
describe('removePanelFromGroup', () => {
  it('covers removePanelFromGroup scenarios', async () => {
    {
      // Scenario: removes a panel from the right group
    ...
    {
      // Scenario: activates the right-adjacent tab after removal
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
    ...
    {
      // Scenario: can activate the left-adjacent tab after removal
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
      state = addPanelToGroup(state, 'b', 'right');
      state = addPanelToGroup(state, 'c', 'right');
    ...
    {
      // Scenario: falls back to the right-adjacent tab when left preference has no left neighbor
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
      state = addPanelToGroup(state, 'b', 'right');
    ...
    {
      // Scenario: activates the left-adjacent tab when no right neighbor
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
      state = addPanelToGroup(state, 'b', 'right');
      state = addPanelToGroup(state, 'c', 'right');
    ...
    {
      // Scenario: clears activeTab when the last tab is removed from a docked group
      let state = createInitialTabGroupState();
    ...
    {
      // Scenario: destroys empty floating groups
      let state = createInitialTabGroupState();
    ...
    {
      // Scenario: removes the correct panel from a floating group with multiple tabs
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'f1', 'floating');
    ...
    {
      // Scenario: returns state unchanged when panel is not found
      const state = createInitialTabGroupState();
    ...
    {
      // Scenario: does not mutate the original state
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
...
describe('setActiveTab', () => {
  it('covers setActiveTab scenarios', async () => {
    {
      // Scenario: sets the active tab in the right group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
      state = addPanelToGroup(state, 'b', 'right');
    ...
    {
      // Scenario: sets the active tab in the bottom group
      let state = createInitialTabGroupState();
    ...
    {
      // Scenario: sets the active tab in a floating group by groupId
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'f1', 'floating');
      state = addPanelToFloatingGroup(state, 'f2', state.floating[0].groupId);
...
describe('getGroupForPanel', () => {
  it('covers getGroupForPanel scenarios', async () => {
    {
      // Scenario: returns "right" for a panel in the right group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
    ...
    {
      // Scenario: returns "bottom" for a panel in the bottom group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'bottom');
    ...
    {
      // Scenario: returns the floating groupId for a panel in a floating group
      let state = createInitialTabGroupState();
    ...
    {
      // Scenario: returns null when the panel is not in any group
      const state = createInitialTabGroupState();
...
// ---------------------------------------------------------------------------
describe('reorderTab', () => {
  it('covers reorderTab scenarios', async () => {
    {
      // Scenario: moves a tab to a new index within the right group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
      state = addPanelToGroup(state, 'b', 'right');
      state = addPanelToGroup(state, 'c', 'right');
    ...
    {
      // Scenario: moves a tab to the beginning
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
      state = addPanelToGroup(state, 'b', 'right');
      state = addPanelToGroup(state, 'c', 'right');
    ...
    {
      // Scenario: reorders tabs in a floating group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'f1', 'floating');
      const groupId = state.floating[0].groupId;
...
describe('movePanelToGroup', () => {
  it('covers movePanelToGroup scenarios', async () => {
    {
      // Scenario: moves a panel from right to bottom
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
    ...
    {
      // Scenario: moves a panel from bottom to right
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'bottom');
    ...
    {
      // Scenario: moves a panel from right to a new floating group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
    ...
    {
      // Scenario: moves a panel into a specific existing floating group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'f1', 'floating');
      const groupId = state.floating[0].groupId;
      state = addPanelToGroup(state, 'a', 'right');
    ...
    {
      // Scenario: activates adjacent tab in the source group after moving
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
      state = addPanelToGroup(state, 'b', 'right');
      state = addPanelToGroup(state, 'c', 'right');
...
describe('addPanelToFloatingGroup', () => {
  it('covers addPanelToFloatingGroup scenarios', async () => {
    {
      // Scenario: adds a panel to an existing floating group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'f1', 'floating');
      const groupId = state.floating[0].groupId;
    ...
    {
      // Scenario: inserts at a specific index
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'f1', 'floating');
      const groupId = state.floating[0].groupId;
      state = addPanelToFloatingGroup(state, 'f2', groupId);
    ...
    {
      // Scenario: removes the panel from its previous group first
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
      state = addPanelToGroup(state, 'f1', 'floating');
      const groupId = state.floating[0].groupId;
    ...
    {
      // Scenario: falls back to a new floating group when the target floating group is missing
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
...
describe('getGroupTabs', () => {
  it('covers getGroupTabs scenarios', async () => {
    {
      // Scenario: returns tabs and activeTab for the right group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'a', 'right');
      state = addPanelToGroup(state, 'b', 'right');
    ...
    {
      // Scenario: returns tabs and activeTab for the bottom group
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'x', 'bottom');
    ...
    {
      // Scenario: returns tabs and activeTab for a floating group by groupId
      let state = createInitialTabGroupState();
      state = addPanelToGroup(state, 'f1', 'floating');
      const groupId = state.floating[0].groupId;
      state = addPanelToFloatingGroup(state, 'f2', groupId);
    ...
    {
      // Scenario: returns empty tabs and null activeTab for an empty docked group
      const state = createInitialTabGroupState();
      const result = getGroupTabs(state, 'right');
    ...
    {
      // Scenario: returns null for an unknown group key
      const state = createInitialTabGroupState();
