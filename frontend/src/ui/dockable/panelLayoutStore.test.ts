...
describe('createPanelLayoutStore — tabGroups slice', () => {
  it('covers createPanelLayoutStore — tabGroups slice scenarios', async () => {
    {
      // Scenario: starts with an empty tabGroups state
      const store = createPanelLayoutStore();
      expect(store.getTabGroups()).toEqual(createInitialTabGroupState());
    }

    {
      // Scenario: setTabGroups applies the updater and updates getTabGroups
      const store = createPanelLayoutStore();
    ...
    {
      // Scenario: subscribeTabGroups notifies listeners on change
      const store = createPanelLayoutStore();
    ...
    {
      // Scenario: subscribeTabGroups returns an unsubscribe function
      const store = createPanelLayoutStore();
      const listener = vi.fn();
    ...
    {
      // Scenario: setTabGroups bails out when the updater returns the same reference
      const store = createPanelLayoutStore();
      const listener = vi.fn();
      store.subscribeTabGroups(listener);
    ...
    {
      // Scenario: tabGroups slice is independent of per-panel state subscriptions
      const store = createPanelLayoutStore();
    ...
    {
      // Scenario: each store instance owns an independent tabGroups slice
    ...
    {
      // Scenario: clearPanelState removes the panel from tab groups
      const store = createPanelLayoutStore();
    ...
    {
      // Scenario: clearPanelState notifies tab group subscribers when it removes grouped panels
      const store = createPanelLayoutStore();
      const listener = vi.fn();
    ...
    {
      // Scenario: restores persisted maximized state with the rest of panel layout state
      const store = createPanelLayoutStore();
    ...
    {
      // Scenario: hands off leader geometry to the next tab before the leader is cleared
      const store = createPanelLayoutStore();
