...
describe('navigateToFavorite', () => {
  it('covers navigateToFavorite scenarios', async () => {
    {
      // Scenario: uses clusterId to avoid reactivating an already active favorite cluster
    ...
    {
      // Scenario: does not reopen a stale favorite path when the same clusterId is already active
      const openKubeconfig = vi.fn().mockResolvedValue(undefined);
      const setActiveKubeconfig = vi.fn();
    ...
    {
      // Scenario: can resolve an open cluster selection from a persisted clusterId
      const openKubeconfig = vi.fn().mockResolvedValue(undefined);
      const setActiveKubeconfig = vi.fn();
      const setPendingFavorite = vi.fn();
      ...
      navigateToFavorite(favorite, {
        ...
        getClusterMeta: (selection) =>
          selection === '/kube/beta:prod'
            ? { id: 'beta:prod', name: 'prod' }
            : { id: '', name: '' },
        setPendingFavorite,
      });
    ...
    {
      // Scenario: does not issue a delayed activation after opening a favorite cluster
      const openKubeconfig = vi.fn().mockResolvedValue(undefined);
      const setActiveKubeconfig = vi.fn();
      const setPendingFavorite = vi.fn();
    ...
    {
      // Scenario: does not activate a cluster for a legacy Global favorite
      const openKubeconfig = vi.fn().mockResolvedValue(undefined);
      const setActiveKubeconfig = vi.fn();
      const setPendingFavorite = vi.fn();
