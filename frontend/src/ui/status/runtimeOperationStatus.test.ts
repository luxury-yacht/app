...
describe('runtimeOperationStatusReducer', () => {
  it('covers runtimeOperationStatusReducer scenarios', async () => {
    {
      // Scenario: keeps workflow detail lists before the runtime operation registry loads
    ...
    {
      // Scenario: uses runtime operations as the active presence envelope
    ...
    {
      // Scenario: applies port-forward status details without changing registry presence
      const state: RuntimeOperationStatusState = {
    ...
    {
      // Scenario: does not resurrect missing port-forward details from status-only events
      const state: RuntimeOperationStatusState = {
        operationsLoaded: true,
        operations: [
          {
...
describe('selectRuntimeOperationRows', () => {
  it('covers selectRuntimeOperationRows scenarios', async () => {
    {
      // Scenario: shows workflow detail rows before the runtime operation registry loads
      const state: RuntimeOperationStatusState = {
        operationsLoaded: false,
        operations: [],
        shellSessions: [
    ...
    {
      // Scenario: filters pre-load workflow detail rows by selected cluster
      const state: RuntimeOperationStatusState = {
        operationsLoaded: false,
        operations: [],
        shellSessions: [
    ...
    {
      // Scenario: uses selected-cluster runtime operations as the presence envelope after load
      const state: RuntimeOperationStatusState = {
        operationsLoaded: true,
        operations: [
          {
