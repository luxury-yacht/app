...
describe('toPlainReleaseNotes', () => {
  it('covers toPlainReleaseNotes scenarios', async () => {
    // Scenario: returns empty string for empty input
    expect(toPlainReleaseNotes('')).toBe('');
    // Scenario: strips heading markers, keeping the text
    expect(toPlainReleaseNotes('## Highlights')).toBe('Highlights');
    expect(toPlainReleaseNotes('#### Deep heading')).toBe('Deep heading');
    // Scenario: removes bold, inline code, and strikethrough markers
    ...
    // Scenario: reduces links to their text and drops images
