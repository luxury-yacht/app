...
describe('resolveBuiltinGroupVersion', () => {
  it('covers resolveBuiltinGroupVersion scenarios', async () => {
    {
      // Scenario: resolves every backend-owned built-in resource identity
    ...
    // Scenario: uses the newest backend-owned default for built-ins with multiple versions
    expect(resolveBuiltinGroupVersion('HorizontalPodAutoscaler')).toEqual({
      group: 'autoscaling',
      version: 'v2',
    });
    // Scenario: resolves kinds case-insensitively
...
describe('parseApiVersion', () => {
  it('covers parseApiVersion scenarios', async () => {
    ...
    // Scenario: handles CRD group/version strings without losing dotted groups
    ...
    // Scenario: trims surrounding whitespace before parsing
    expect(parseApiVersion('  apps/v1  ')).toEqual({ group: 'apps', version: 'v1' });
  });
});
