...
describe('parsePlanCount', () => {
  it('covers parsePlanCount scenarios', async () => {
    // Scenario: parses Evicting and Deleting messages
    expect(parsePlanCount('Evicting 12 pods')).toBe(12);
    expect(parsePlanCount('Deleting 1 pod')).toBe(1);
    // Scenario: returns undefined when the message is missing or unrecognised
...
describe('deriveDrainProgress', () => {
  it('covers deriveDrainProgress scenarios', async () => {
    {
      // Scenario: rolls pod events up into per-pod rows ordered in-progress, failed, done
    ...
    // Scenario: marks phase as cordoning, planning, evicting, waiting based on event order
    expect(
    ...
    {
      // Scenario: forces phase=completed for terminal jobs even without a wait-complete event
      const result = deriveDrainProgress(
        job({
          status: 'succeeded',
          completedAt: 2000,
    ...
    {
      // Scenario: falls back to totalSeen when the plan event is absent
      const result = deriveDrainProgress(
        job({
          events: [
            evt({
    ...
    {
      // Scenario: treats failed job status as hasError even with no error events
    ...
    {
      // Scenario: copies the timeoutSeconds option through for UI display
