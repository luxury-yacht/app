...
export type TabDragPayload =
  | { kind: 'cluster-tab'; clusterId: string; selection: string; sourceWindowName: string }
  | {
      kind: 'dockable-tab';
      panelId: string;
...
export const tabDragScopeDataType = (scope: TabDragScope): string => {
  const bytes = new TextEncoder().encode(JSON.stringify(scope.clusterId));
...
export const tabDragMatchesScope = (payload: TabDragPayload, scope: TabDragScope): boolean =>
  payload.kind === 'dockable-tab' && payload.clusterId === scope.clusterId;
