...
describe('diffUtils', () => {
  it('covers diffUtils scenarios', async () => {
    // Scenario: keeps context rows aligned on both sides
    expect(
    ...
    {
      // Scenario: pairs remove/add blocks and preserves unbalanced line numbers
    ...
    {
      // Scenario: pairs change blocks by side order even when additions arrive first
      const lines: DiffLine[] = [
    ...
    // Scenario: compares truncation maps by row and side
    expect(
      areTruncationMapsEqual(
    ...
    {
      // Scenario: counts full and changed-only rows
    ...
    // Scenario: formats the render-budget message with localized counts
