...
describe('Dropdown', () => {
  ...
  it('positions menu upward when inside a clipping scroll container with limited space below', async () => {
    const originalInnerHeight = window.innerHeight;
    ...
    await mount(
      <div data-testid="clipper" style={{ overflowY: 'auto', maxHeight: '300px' }}>
        <Dropdown options={OPTIONS} value="" onChange={vi.fn()} />
      </div>
    );

    const clipper = container.querySelector('[data-testid="clipper"]') as HTMLElement;
    Object.defineProperty(clipper, 'getBoundingClientRect', {
      configurable: true,
      value: () => ({
        top: 100,
        bottom: 320,
