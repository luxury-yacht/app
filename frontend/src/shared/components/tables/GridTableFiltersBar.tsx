...
function queryFacetChipType(facet: GridTableQueryFacetDefinition): string {
	...
	const unrestrictedLabel =
		hasAllPrefix && hasWhitespaceSeparator && trimmedSuffix
			? trimmedSuffix
			: facet.label;
	return unrestrictedLabel.charAt(0).toUpperCase() + unrestrictedLabel.slice(1);
}

function queryFacetChipSingularType(
	facet: GridTableQueryFacetDefinition,
): string {
...
function renderResultCountChip(
	...
	return (
		<span
			className="active-filter-chips__summary"
						...
						<>
							{resultCount.totalIsExact === false && (
								<p className="gridtable-filter-result-tooltip-paragraph">
									The total count is approximate because the backend stopped
									counting after the configured exact-count budget.
								</p>
							)}
...
interface ColumnsDropdownOptions {
	...
	renderColumnsValue: NonNullable<
		GridTableFiltersBarProps["renderColumnsValue"]
	>;
}
...
const GridTableFiltersBar: React.FC<GridTableFiltersBarProps> = ({
	...
	const filterControls: ResolvedMultiselectFilterControl[] = [
		...
		...queryFacets.map((facet): ResolvedMultiselectFilterControl => {
			const selection =
				activeFilters.queryFacets?.[facet.key] ?? ALL_MULTISELECT_FILTER;
			const count = selection.mode === "some" ? selection.values.length : 0;
			return {
				...
				renderValue: () =>
					count > 0 ? `${facet.label} (${count})` : facet.label,
			};
		}),
	];
	...
	const controlsAt = (
		placement: FilterControlPlacement,
	): ResolvedMultiselectFilterControl[] =>
		filterControls.filter(
			(control) => control.visible && control.placement === placement,
		);
	...
	return (
		<div className="gridtable-filter-container">
			<div className="gridtable-filter-bar" ref={containerRef}>
				<div
					className="gridtable-filter-cluster"
					data-gridtable-filter-cluster="primary"
				>
					...
					<div className="gridtable-filter-subcluster">
						...
						>
							<SearchInput
								...
								placeholder={
									resolvedFilterOptions.searchPlaceholder ?? "Filter"
								}
				...
				<div
					className="gridtable-filter-cluster"
					data-gridtable-filter-cluster="tertiary"
				>
