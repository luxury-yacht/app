...
vi.mock("@ui/shortcuts", () => ({
	useSearchShortcutTarget: (config: unknown) =>
		searchShortcutMock.register(config),
}));
	...
	Dropdown: ({
		...
		<div className={dropdownClassName}>
			...
			{options.map((option) => (
				...
				>
					{renderOption
						? renderOption(option, value.includes(option.value))
						: option.label}
...
describe("GridTableFiltersBar", () => {
	...
	it("renders dropdowns and propagates changes", async () => {
		...
		const kindDropdown = container.querySelector(
			'[data-testid="kinds"]',
		...
		const nsDropdown = container.querySelector(
	...
	it("renders built-in multiselect chips from the stored selection modes", async () => {
		...
		const chips = container.querySelector(
			'[aria-label="Active GridTable filters"]',
		);
		...
		await act(async () => {
			container
				.querySelector<HTMLButtonElement>(
					'button[aria-label="Clear Kinds filter"]',
				)
				?.click();
		});
	...
	it("renders leading query facets before Kinds", async () => {
		...
		const controls = Array.from(
			container.querySelectorAll<HTMLSelectElement>(
				".gridtable-filter-subcluster select",
			),
	...
	it("renders structural actions immediately before Namespaces", async () => {
		...
		const controls = Array.from(
			container.querySelector('[data-gridtable-filter-cluster="primary"]')
				?.children[0]?.children ?? [],
	...
	it("always enables search and bulk actions for the kind dropdown", async () => {
		...
		expect(
			container
				.querySelector('[data-testid="kinds"]')
				?.getAttribute("data-searchable"),
		).toBe("true");
		expect(
			container
				.querySelector('[data-testid="kinds"]')
				?.getAttribute("data-bulk-actions"),
		).toBe("true");
	});
	...
	it("passes searchable through to the namespace dropdown when enabled", async () => {
		...
		expect(
			container
				.querySelector('[data-testid="namespaces"]')
				?.getAttribute("data-searchable"),
		).toBe("true");
	});
	...
	it("keeps search input focused across controlled filter updates", async () => {
		...
		await act(async () => {
			...
			setInputValue(
				requireValue(
					input,
					"expected test value in GridTableFiltersBar.test.tsx",
				),
				"p",
			);
			await Promise.resolve();
	...
	it("hides the case-sensitive toggle for query-backed search", async () => {
		...
		expect(
			container.querySelector('.icon-bar-button[title="Match case"]'),
		).toBeNull();
	});
	...
	it("shows the result count once a narrowing filter is active", async () => {
		...
		expect(resultCount.classList.contains("active-filter-chips__summary")).toBe(
			true,
		);
		expect(resultCount.classList.contains("active-filter-chip")).toBe(false);
		...
		expect(
			Boolean(
				resultCount.compareDocumentPosition(clearAll) &
	...
	it("surfaces the partial-window note in the result-count tooltip", async () => {
		...
		expect(document.body.textContent).toContain(
			"Only the recent local window is loaded.",
		);
	...
	it("does not add a leading separator when only post-actions are rendered", async () => {
		...
		expect(
			container.querySelector('.icon-bar-button[title="Load more"]'),
		).not.toBeNull();
	...
	it("renders the columns dropdown when enabled", async () => {
		...
		expect(
			container.querySelectorAll(".gridtable-column-drag-handle"),
		).toHaveLength(2);
		...
		await act(async () =>
			ageRow.dispatchEvent(new Event("dragend", { bubbles: true })),
		);
	...
	it("adds, edits, and directly deletes custom metadata columns from the Columns menu", async () => {
		...
		await act(async () => {
			container
				.querySelector<HTMLButtonElement>('button[aria-label="Edit Owner"]')
				?.click();
		});
		expect(onEditCustomMetadataColumn).toHaveBeenCalledWith(
			"metadata:label:example.com/owner",
		);

		expect(
			container.querySelector('button[aria-label="Delete Name"]'),
		).toBeNull();
		await act(async () => {
			container
				.querySelector<HTMLButtonElement>('button[aria-label="Delete Owner"]')
				?.click();
		});
		expect(onRemoveCustomMetadataColumn).toHaveBeenCalledWith(
			"metadata:label:example.com/owner",
		);
	});
