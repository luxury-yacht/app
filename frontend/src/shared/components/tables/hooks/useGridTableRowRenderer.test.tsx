...
describe('useGridTableRowRenderer', () => {
  ...
  it('covers useGridTableRowRenderer scenarios', async () => {
    {
      // Scenario: renders rows with virtualization applied
    ...
    {
      // Scenario: renders plain rows when virtualization disabled
      const measureRowRef = vi.fn();
      const handleRowClick = vi.fn();
      const renderers = renderHook(() =>
        useGridTableRowRenderer({
    ...
    {
      // Scenario: supports right-aligned data independently from header alignment
    ...
    {
      // Scenario: does not assign native title tooltips to grid cells
      const renderers = renderHook(() =>
        useGridTableRowRenderer({
          keyExtractor: (_item, index) => `row-${index}`,
          getRowClassName: () => '',
