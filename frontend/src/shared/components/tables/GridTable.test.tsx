...
type RenderOptions = Partial<{
	...
	customMetadataColumns: NonNullable<
		GridTableProps<SimpleRow>["customMetadataColumns"]
	>;
}>;
...
describe("GridTable virtualization", () => {
	...
	it("renders backend query windows without an extra client cap", () => {
		...
		expect(renderedRows).toHaveLength(8);
		expect(container.textContent).toContain("Row 7");
		const resultCount = container.querySelector(
			'[data-gridtable-filter-role="result-count"]',
		);
		expect(resultCount?.textContent).toBe("Showing 20 of 20 items");
		expect(
			resultCount?.classList.contains("active-filter-chips__summary"),
		).toBe(true);
	...
	it("renders the Copy · Export pair acting on all matching rows (no scope toggle)", () => {
		...
		const exportBtn = container.querySelector(
			'[aria-label="Export all matching rows to file"]',
		);
	...
	it("updates the rendered slice when scrolling", async () => {
		...
		const renderedRows = Array.from(
			container.querySelectorAll(".gridtable-row"),
	...
	it("expands the virtual body width to match the total column width", () => {
		...
		const virtualBody = container.querySelector<HTMLElement>(
			".gridtable-virtual-body",
		);

		expect(virtualBody).not.toBeNull();
		expect(
			requireValue(virtualBody, "expected test value in GridTable.test.tsx")
				.style.width,
		).toBe("800px");
	});
	...
	it("keeps a horizontal scroll viewport outside the wide native table", () => {
		...
		const grid = viewport?.querySelector<HTMLTableElement>(
		...
		expect(
			grid?.querySelector<HTMLElement>(".gridtable-virtual-body")?.style.width,
		).toBe("800px");
	});
	...
	it("updates virtual height when the first rendered row grows taller", async () => {
		...
		try {
			let virtualBody = container.querySelector<HTMLDivElement>(
				".gridtable-virtual-body",
			);
			expect(virtualBody).not.toBeNull();
			...
			virtualBody = container.querySelector<HTMLDivElement>(
				".gridtable-virtual-body",
			);
			...
			const updatedBody = container.querySelector<HTMLDivElement>(
				".gridtable-virtual-body",
			);
...
describe("GridTable interactions (non-virtualized)", () => {
	it("renders initial loading state when no data is available", async () => {
		...
		expect(
			container.querySelector(".gridtable-initial-loading"),
		).not.toBeNull();
		expect(
			container.querySelector(".gridtable-initial-loading")?.textContent,
	...
	it("does not invoke row click handler for pointer clicks on the row body", async () => {
		...
		expect(
			requireValue(
				firstRow,
				"expected test value in GridTable.test.tsx",
			).classList.contains("gridtable-row--focused"),
		).toBe(true);
	});
	...
	it("clears row selection only when unused body space is clicked", async () => {
		...
		await act(async () => {
			requireValue(
				firstRow,
				"expected row in GridTable background click test",
			).click();
		});
		...
		await act(async () => {
			requireValue(
				wrapper,
				"expected wrapper in GridTable background click test",
			).click();
		});
	...
	it("focuses the row when clicking opt-in interactive elements", async () => {
		...
		const interactive =
			container.querySelector<HTMLSpanElement>(".gridtable-link");
	...
	it("preserves scroll position when dismissing the context menu with a pointer click", async () => {
		...
		const wrapper =
			container.querySelector<HTMLDivElement>(".gridtable-wrapper");
		const grid = container.querySelector<HTMLTableElement>(
			"table.gridtable--body",
		);
		expect(wrapper).not.toBeNull();
		expect(grid).not.toBeNull();
	...
	it("renders filter controls and propagates search changes", async () => {
		...
		const expectedMatches = createRows(30).filter((row) =>
			row.label.includes("Row 1"),
		).length;
		expect(visibleRows.length).toBe(expectedMatches);
		expect(
			Array.from(visibleRows).every((row) =>
				row.textContent?.includes("Row 1"),
			),
		).toBe(true);
	...
	it("removes row-internal controls from the tab order so the grid stays the only body tab stop", async () => {
		...
		expect(grid).not.toBeNull();
		expect(
			requireValue(rowButton, "expected test value in GridTable.test.tsx")
				.tabIndex,
		).toBe(-1);
		expect(
			requireValue(grid, "expected test value in GridTable.test.tsx").tabIndex,
		).toBe(0);
	});
...
function renderGridTable(options: RenderOptions = {}) {
	...
	const initialProps = {
		...
		keyExtractor:
			options.keyExtractor ?? ((item: SimpleRow) => `cluster-a|${item.id}`),
		virtualization: {
			enabled: true,
			threshold: 1,
			overscan: 1,
	...
	act(() => {
		root.render(
			<ZoomProvider>
				<KeyboardProvider>
					<GridTable<SimpleRow>
						{...(currentProps as GridTableProps<SimpleRow>)}
					/>
				</KeyboardProvider>
			</ZoomProvider>,
		);
...
it("appends configured metadata columns to the rendered and exportable column set", () => {
	...
	expect(container.querySelector(".gridtable--header")?.textContent).toContain(
		"Owner",
	);
...
it("creates a custom metadata column from the Columns menu", async () => {
	...
	expect(
		document.body.querySelector('dialog[aria-label="Columns"]'),
	).toBeNull();
	await act(async () => {
		requireValue(
			document.body.querySelector<HTMLButtonElement>(
...
it("deletes a custom metadata column directly from the Columns menu", async () => {
	...
	await act(async () => {
		requireValue(
			document.body.querySelector<HTMLButtonElement>(
				'button[aria-label="Delete Owner"]',
			),
...
it("renders paginationControls in the footer without any pagination callbacks", () => {
	const { container, cleanup } = renderGridTable({
		...
		virtualization: { enabled: false },
		paginationControls: (
			<div data-testid="cursor-pagination-controls">controls</div>
		),
	});
	cleanupRoot = cleanup;
	...
	expect(
		container.querySelector('[data-testid="cursor-pagination-controls"]'),
	).not.toBeNull();
...
it("paginates a local row set after the table pipeline and renders exact footer controls", async () => {
	...
	expect(
		container.querySelector(".gridtable-pagination")?.textContent,
	).toContain("1-2 of 5");
...
it("copies the current visible table contents as CSV from the filter icon bar", async () => {
	...
	const csvColumns: GridColumnDefinition<
		SimpleRow & { notes: string; secret: string }
	>[] = [
		{
			key: "label",
			header: "Label",
...
it("shows cell-level context menu items for the targeted row", async () => {
	...
	expect(customItems).toHaveBeenCalledWith(
		expect.objectContaining({ id: "row-0" }),
		"label",
	);
	...
	expect(
		requireValue(menu, "expected test value in GridTable.test.tsx").textContent,
	).toContain("Inspect Row 0");
...
it("filters rows using an initial search filter", () => {
	...
	const betaRows = filteredRows.filter((row) =>
		row.textContent?.includes("Beta"),
	);
	const alphaRows = filteredRows.filter((row) =>
...
it("renders filter UI when enabled", () => {
	...
	expect(
		container.querySelector('input[name="gridtable-filter-search"]'),
	).not.toBeNull();
...
it("does not warn when keyExtractor returns a cluster-scoped key", async () => {
	...
	const clusterWarnings = warnSpy.mock.calls.filter(
		(args) =>
			...
			args[0].includes("does not appear cluster-scoped"),
	);
...
it("returns auto-width columns to automatic sizing from the Columns reset action", async () => {
	...
	await act(async () => {
		requireValue(
			columnsTrigger,
			"expected Columns trigger in GridTable.test.tsx",
		).click();
		await Promise.resolve();
	});
