...
export default function CustomMetadataColumnEditor({
	...
	const normalizedHeader =
		header.trim() || defaultCustomMetadataColumnHeader(normalizedKey);
	...
	const canSave =
		candidate !== null && normalizedHeader.length > 0 && !duplicate;
	const editorKeys =
		state.mode === "edit" &&
		!availableKeys.some(
			(key) =>
				key.source === state.definition.source &&
				key.metadataKey === state.definition.metadataKey,
		)
			? [
					...availableKeys,
					{
		...
		return [
			...
			...sourceKeys.map((key) => ({
				...
				disabled:
					...
					definitions.some(
						(definition) =>
							definition.source === key.source &&
							definition.metadataKey === key.metadataKey,
					),
			})),
		];
	});
	const selectedOptionValue = normalizedKey
		? `metadata:${source}:${normalizedKey}`
		: "";
		...
		>
			...
			>
				<div className="modal-content modal-form custom-metadata-column-editor__content">
					<div>
						Select a metadata key (Label or Annotation) to use as a custom
						column
					</div>
					<div className="custom-metadata-column-editor__field">
						...
						<Dropdown
							...
							onChange={(value) => {
								...
								if (!headerEdited) {
									setHeader(
										defaultCustomMetadataColumnHeader(nextMetadata.metadataKey),
									);
								}
							}}
							displayValue={(value) => {
						...
						{!!duplicate && (
							<span className="modal-field-message modal-field-error">
								Already added
							</span>
						)}
					</div>
					...
					<label className="custom-metadata-column-editor__field">
						...
						<input
							...
							placeholder={
								normalizedKey
									? defaultCustomMetadataColumnHeader(normalizedKey)
									: "Owner"
							}
							data-modal-initial-focus={
								state.mode === "edit" ? true : undefined
							}
							onChange={(event) => {
								setHeaderEdited(true);
					...
					{state.mode === "edit" && (
						<div className="custom-metadata-column-editor__stable-id">
							Renaming keeps the column’s width, order, visibility, and favorite
							reference.
						</div>
					)}
				</div>
					...
					{state.mode === "edit" && (
						<button
							...
							onClick={() => {
								onChange(
									definitions.filter(
										(definition) => definition.key !== state.definition.key,
									),
								);
								onClose();
							}}
