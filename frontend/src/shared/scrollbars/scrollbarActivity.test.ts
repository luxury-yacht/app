...
describe('scrollbar activity tracking', () => {
  ...
  it('activates shell session scrollbars on wheel events', () => {
    ...
    expect(terminal.classList.contains('shell-tab__terminal--scrollbar-hover-vertical')).toBe(
      false
    );
  ...
  it('activates shell session scrollbar hover zones from pointer position', () => {
    const terminal = document.createElement('div');
    terminal.className = 'shell-tab__terminal';
    ...
    const elementsFromPoint = vi
      ...
      .mockReturnValue([scrollbar, terminal]);

    dispatchPointerMove(99, 50);

    expect(terminal.classList.contains('scrollbar-active')).toBe(true);
    expect(terminal.classList.contains('shell-tab__terminal--scrollbar-hover-vertical')).toBe(true);

    elementsFromPoint.mockReturnValue([terminal]);
