...
describe('getMetricsBannerInfo', () => {
  it('covers getMetricsBannerInfo scenarios', async () => {
    {
      // Scenario: reports "Collecting metrics" for the pristine first-collection window
    ...
    {
      // Scenario: surfaces the terminal reason for a disabled poller instead of "Collecting metrics"
    ...
    {
      // Scenario: returns awaiting message during initial transient failures
      const banner = getMetricsBannerInfo({
    ...
    {
      // Scenario: surfaces errors after repeated failures
      const banner = getMetricsBannerInfo({
        lastError: 'metrics API unavailable (pods.metrics.k8s.io)',
        successCount: 0,
        failureCount: 5,
    ...
    {
      // Scenario: returns null when metrics are fresh with no errors
      const banner = getMetricsBannerInfo({
    ...
    {
      // Scenario: passes through real errors after successes
      const banner = getMetricsBannerInfo({
        lastError: 'metrics API unavailable (pods.metrics.k8s.io)',
        successCount: 1,
        failureCount: 2,
