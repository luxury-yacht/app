...
// (built with nil slices, so it never carries the Endpoints segment); this re-join appends
// the Endpoints segment for the ready endpoint count, using the SAME
// service.AppendEndpointsSegment the typed path uses, so the result is identical. The
// append copies the segment slice — the own-row is served from the shared maintained
// store, so an in-place append would corrupt concurrently served rows.
//
...
func reaggregateServiceSummary(own NetworkSummary, readyEndpointCount int) NetworkSummary {
	summary := own
	summary.Details = service.AppendEndpointsSegment(own.Details, readyEndpointCount)
	return summary
}
