...
func TestGovernorPolicyAssign(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			mru:  []string{"a"}, visible: map[string]bool{"a": true}, keepWarm: 2,
			want: map[string]ResourceTier{"a": TierForeground},
		},
		{
			name: "warm budget keeps the next N background, rest cold",
			mru:  []string{"a", "b", "c", "d"}, visible: map[string]bool{"a": true}, keepWarm: 2,
		...
		},
		{
			...
			mru:  []string{"b", "a", "c"}, visible: map[string]bool{"a": true}, keepWarm: 1,
		...
		},
		{
			...
			mru:  []string{"a", "b", "c"}, visible: map[string]bool{"a": true}, keepWarm: 5, underPressure: true,
			want: map[string]ResourceTier{"a": TierForeground, "b": TierCold, "c": TierCold},
		},
		{
			name: "keepWarm zero demotes every non-visible cluster",
			mru:  []string{"a", "b", "c"}, visible: map[string]bool{"a": true}, keepWarm: 0,
			want: map[string]ResourceTier{"a": TierForeground, "b": TierCold, "c": TierCold},
		},
		{
			name: "no visible cluster (e.g. app blurred): all subject to the warm budget",
			mru:  []string{"a", "b", "c"}, visible: nil, keepWarm: 1,
			want: map[string]ResourceTier{"a": TierBackground, "b": TierCold, "c": TierCold},
		},
		{
			name: "every cluster visible in a peer window remains foreground",
			mru:  []string{"a", "b", "c"}, visible: map[string]bool{"a": true, "c": true}, keepWarm: 0,
			want: map[string]ResourceTier{"a": TierForeground, "b": TierCold, "c": TierForeground},
		},
	}
	for _, c := range cases {
