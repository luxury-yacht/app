...
func TestBuildEndpointSliceResourceModelFactsAndStatus(t *testing.T) {
	...
	require.Equal(t, "cluster-a", model.Facts.EndpointSlice.NotReadyAddresses[0].TargetRef.Display.ClusterID)
	require.Equal(t, "v1", model.Facts.EndpointSlice.NotReadyAddresses[0].TargetRef.Display.Version)
	require.Equal(t, "Pod", model.Facts.EndpointSlice.NotReadyAddresses[0].TargetRef.Display.Kind)
	require.Equal(t, "default", model.Facts.EndpointSlice.NotReadyAddresses[0].TargetRef.Display.Namespace)
	require.Equal(t, "api-0", model.Facts.EndpointSlice.NotReadyAddresses[0].TargetRef.Display.Name)
