...
Linux use frameless workspace and panel windows; `WindowHeader` supplies the drag
surface and window controls. The workspace `AppHeader` composes that shared chrome
with the application menu, status indicators, favorites, and command palette.
Panel renderers import the shared chrome directly so their startup does not load
workspace-only controls. Windows leaves WebView2 non-client region support and
composition hosting disabled so pointer input reaches Wails' DOM resize-before-drag handler.
