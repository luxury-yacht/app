...
into the desktop-service context. The shell resolves that sender through the
native-window registry, rejects panel commands before the panel is live, keeps
window-local commands in the sender, and routes workspace-owned commands from a
panel to its immutable owner.
...
| Platform window chrome and menus | `windowOptionsForPlatform`, `panelWindowOptionsForPlatform`, and `backend.DesktopShell.ExecuteApplicationMenuCommand` | No | The native menu is created before windows for macOS. Windows/Linux windows are frameless and render controls in `AppHeader`; workspace windows additionally render `AppMenuBar`. Panel accelerators register only after the panel's native ready acknowledgement. | macOS owns the application menu and native traffic lights. Windows/Linux install no native window menu. App-rendered accelerators carry Wails sender identity; the registry classifies each registered live sender so commands remain local, route to the panel's immutable owner, or act process-wide. Native and app-rendered items share the typed dispatcher. Proof: `internal/appwindow/registry.go`, `backend/application_menu_commands.go`, `frontend/src/ui/layout/AppMenuBar.tsx`, `frontend/src/ui/shortcuts/components/ApplicationMenuShortcuts.tsx`, `frontend/src/ui/layout/AppHeader.tsx`, and their focused tests. |
| Window-local zoom accelerators | Native macOS menu accelerators and frontend Windows/Linux shortcuts both reach the shared zoom events | No | The frontend zoom context remains the action owner in each renderer | Menu commands target the authenticated calling workspace or panel; no shutdown work. Proof: `backend/menu.go`, `backend/application_menu_commands.go`, `frontend/src/ui/shortcuts/components/ApplicationMenuShortcuts.tsx`, and `frontend/src/core/contexts/ZoomContext.tsx`. |
