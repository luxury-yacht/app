...
| Panel-window close | A panel's cancellable native close hook routes a guard request to its child renderer and immutable owner | Yes | The child guards every tab, the owner removes the group from its directory, and only a registry-authorized second close removes the native role. Failed opening transfers notify the owner before discarding the role. | Panel close never releases a workspace or cluster-tab owner. A denial or timeout leaves the panel live. Proof: `internal/appwindow/registry.go`, `internal/appwindow/panel.go`, `internal/appwindow/panel_transfer.go`, and `frontend/src/ui/shortcuts/components/PanelWindowShortcuts.tsx`. |
