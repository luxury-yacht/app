...
Status: Complete. The previous large-table work completed the first production
...
query surfaces, and shared table-mode enforcement. It also made remaining capped
tables more honest, but that does not satisfy the app-wide goal below because
high-cardinality capped tables still do not provide backend-owned access to all
matching rows.
