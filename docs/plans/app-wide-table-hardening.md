...
Status: Complete. The app-wide table hardening work moved high-cardinality
cluster and all-namespaces resource tables to backend-owned query pages, kept
owner/scope-bounded local tables local, made remaining capped/recent/buffered
surfaces visibly `Local Partial`, and added shared table-mode enforcement.
...
Do not recreate the old completed implementation plan. This document records
the completed inventory, guardrails, validation, and Definition of Done that
future table work must preserve.
...
- All-namespaces Namespace Config, Network, Storage, RBAC, Quotas,
  Autoscaling, Helm, and Events: `Query Backed Static`.
- Cluster Nodes: `Query Backed Dynamic`.
- Cluster Config, Storage, RBAC, CRDs, and Events: `Query Backed Static`.

Remaining `Local Partial` surfaces:

- Single-namespace capped snapshots when producer stats report truncation.
- Object Events.
- Parsed logs, where table-like filtering/export is bounded by the log buffer.

Remaining `Local Complete` or conditional surfaces with measured bounds:

- Single-namespace namespace resource snapshots are `Local Complete` when the
  producer stats prove they are not truncated, and `Local Partial` when the
  producer reports truncation.
- Object-panel related Pods and Jobs remain owner-scoped and must not fan out
  to namespace/cluster scale.
...
| Namespace Pods, single namespace      | `namespace-pods` in `NsViewPods`                                      | `pods` refresh snapshot/stream; `PodSnapshotEntry`                                                                              | one namespace for one cluster                                   | Namespace-scoped snapshot; `Local Complete` when producer stats prove it is not truncated and visibly `Local Partial` when truncated                        | Local count/facets/search/sort over the complete or declared partial namespace window                                   | Visible-row CSV and row actions only                                                                                                          | Namespace pod count; can be high in large tenant namespaces                        |
...
| Cluster Events                        | `cluster-events` in `ClusterViewEvents`                               | `backend/refresh/snapshot/cluster_events.go` typed query; `ClusterEventEntry`                                                   | cluster event set for one cluster                               | Backend search/filter/sort/page over the current event set; `Query Backed Static`                                                                           | Backend total/exactness, kind facets, keyset next page                                                                 | Visible-row CSV and involved-object action only                                                                                               | Event volume is unbounded; backend query bounds React to the current page          |
| Namespace Events                      | `namespace-events` in `NsViewEvents`                                  | `backend/refresh/snapshot/namespace_events.go` typed query; `NamespaceEventSummary`                                             | single namespace or all-namespaces event set for one cluster    | Backend search/filter/sort/page over the current event set for all-namespaces; single namespace remains stats-backed local                                  | Backend total/exactness, kind and namespace facets, keyset next page for all-namespaces; local stats for single namespace | Visible-row CSV and involved-object action only                                                                                               | Event volume is unbounded; all-namespaces query bounds React to the current page   |
...
| Namespace Config                      | `namespace-config` in `NsViewConfig`                                  | `backend/refresh/snapshot/namespace_config.go` typed query; `NamespaceConfigSummary`                                            | single namespace or all-namespaces for one cluster              | All-namespaces uses backend search/filter/sort/page; `Query Backed Static`. Single namespace remains stats-backed local                                     | Backend total/exactness, kind and namespace facets, keyset next page for all-namespaces; local stats for single namespace | Visible-row CSV and row actions only                                                                                                          | ConfigMap/Secret count can be high, especially all-namespaces                      |
| Namespace Network                     | `namespace-network` in `NsViewNetwork`                                | `backend/refresh/snapshot/namespace_network.go` typed query; `NamespaceNetworkSummary`                                          | single namespace or all-namespaces for one cluster              | All-namespaces uses backend search/filter/sort/page; `Query Backed Static`. Single namespace remains stats-backed local                                     | Backend total/exactness, kind and namespace facets, keyset next page for all-namespaces; local stats for single namespace | Visible-row CSV and row actions only                                                                                                          | Services/Ingresses/EndpointSlices/policies can be high                             |
| Namespace Storage                     | `namespace-storage` in `NsViewStorage`                                | `backend/refresh/snapshot/namespace_storage.go` typed query; `NamespaceStorageSummary`                                          | single namespace or all-namespaces for one cluster              | All-namespaces uses backend search/filter/sort/page; `Query Backed Static`. Single namespace remains stats-backed local                                     | Backend total/exactness, namespace facets, keyset next page for all-namespaces; local stats for single namespace        | Visible-row CSV and row actions only                                                                                                          | PVC count can be high across namespaces                                            |
| Namespace Quotas                      | `namespace-quotas` in `NsViewQuotas`                                  | `backend/refresh/snapshot/namespace_quotas.go` typed query; `NamespaceQuotaSummary`                                             | single namespace or all-namespaces for one cluster              | All-namespaces uses backend search/filter/sort/page; `Query Backed Static`. Single namespace remains stats-backed local                                     | Backend total/exactness, kind and namespace facets, keyset next page for all-namespaces; local stats for single namespace | Visible-row CSV and row actions only                                                                                                          | Usually moderate, but all-namespaces PDBs can be high                              |
| Namespace Autoscaling                 | `namespace-autoscaling` in `NsViewAutoscaling`                        | `backend/refresh/snapshot/namespace_autoscaling.go` typed query; `NamespaceAutoscalingSummary` -> `AutoscalingData`             | single namespace or all-namespaces for one cluster              | All-namespaces uses backend search/filter/sort/page; `Query Backed Static`. Single namespace remains stats-backed local                                     | Backend total/exactness, kind and namespace facets, keyset next page for all-namespaces; local stats for single namespace | Visible-row CSV and row actions only                                                                                                          | HPA count can grow with workload count                                             |
| Namespace RBAC                        | `namespace-rbac` in `NsViewRBAC`                                      | `backend/refresh/snapshot/namespace_rbac.go` typed query; `NamespaceRBACSummary`                                                | single namespace or all-namespaces for one cluster              | All-namespaces uses backend search/filter/sort/page; `Query Backed Static`. Single namespace is `Local Complete` below cap and `Local Partial` when truncated | Backend total/exactness, kind and namespace facets, keyset next page for all-namespaces; local stats for single namespace | Visible-row CSV and row actions only                                                                                                          | Role/RoleBinding count can be high across namespaces                               |
| Namespace Helm                        | `namespace-helm` in `NsViewHelm`                                      | `backend/refresh/snapshot/namespace_helm.go` typed query; `NamespaceHelmSummary` -> synthetic HelmRelease row                   | single namespace or all-namespaces for one cluster              | All-namespaces uses backend search/filter/sort/page; `Query Backed Static`. Single namespace is `Local Complete` below cap and `Local Partial` when truncated | Backend total/exactness, namespace facets, keyset next page for all-namespaces; local stats for single namespace        | Visible-row CSV and synthetic HelmRelease actions only                                                                                        | Release count can be high across namespaces                                        |
| Cluster Nodes                         | `cluster-nodes` in `ClusterViewNodes`                                 | `backend/refresh/snapshot/nodes.go` typed query; `ClusterNodeRow` with metrics                                                  | cluster for one cluster                                         | Backend search/filter/sort/page with metrics revision; `Query Backed Dynamic`                                                                               | Backend total/exactness, keyset next page, CPU/memory sort tied to dynamic metrics revision                             | Visible-row CSV and row actions only                                                                                                          | Node count is usually bounded, but query path bounds React to the current page     |
| Cluster Config                        | `cluster-config` in `ClusterViewConfig`                               | `backend/refresh/snapshot/cluster_config.go` typed query; `ClusterConfigEntry`                                                  | cluster-scoped config kinds for one cluster                     | Backend search/filter/sort/page; `Query Backed Static`                                                                                                      | Backend total/exactness, kind facets, keyset next page                                                                  | Visible-row CSV and row actions only                                                                                                          | StorageClass/IngressClass/GatewayClass/webhook count can be high in large clusters |
| Cluster Storage                       | `cluster-storage` in `ClusterViewStorage`                             | `backend/refresh/snapshot/cluster_storage.go` typed query; `ClusterStorageEntry`                                                | cluster-scoped PVs for one cluster                              | Backend search/filter/sort/page; `Query Backed Static`                                                                                                      | Backend total/exactness, keyset next page                                                                               | Visible-row CSV and row actions only                                                                                                          | PersistentVolume count can be high                                                 |
| Cluster RBAC                          | `cluster-rbac` in `ClusterViewRBAC`                                   | `backend/refresh/snapshot/cluster_rbac.go` typed query; `ClusterRBACEntry`                                                      | cluster-scoped RBAC for one cluster                             | Backend search/filter/sort/page; `Query Backed Static`                                                                                                      | Backend total/exactness, kind facets, keyset next page                                                                  | Visible-row CSV and row actions only                                                                                                          | ClusterRole/Binding count can be high                                              |
| Cluster CRDs                          | `cluster-crds` in `ClusterViewCRDs`                                   | `backend/refresh/snapshot/cluster_crds.go` typed query; `ClusterCRDEntry`                                                       | cluster-scoped CRDs for one cluster                             | Backend search/filter/sort/page; `Query Backed Static`                                                                                                      | Backend total/exactness, keyset next page                                                                               | Visible-row CSV and row actions only                                                                                                          | CRD count can be high in extension-heavy clusters                                  |
...
Phase 3 migrated the remaining high-cardinality all-namespaces namespace
families and cluster families to backend-owned typed query pages. Namespace and
cluster Events now page over the current backend event set instead of only
rendering a capped frontend window. Nodes remain `Query Backed Dynamic` because
CPU and memory sorting is tied to the metrics revision in the cursor contract.
...
`go test ./backend/refresh/snapshot -run '^$' -bench 'BenchmarkMigratedStaticTableQueries$' -benchtime=1x`,
`npm run test --prefix frontend -- ClusterViewNodes ClusterViewConfig ClusterViewStorage ClusterViewRBAC ClusterViewCRDs ClusterViewEvents NsViewConfig NsViewNetwork NsViewStorage NsViewQuotas NsViewAutoscaling NsViewRBAC NsViewHelm NsViewEvents useTypedResourceQuery typedResourceQueryScope gridTableViewRegistry.contract`,
and `npm run typecheck --prefix frontend`.

Phase 7 first-load validation on 2026-06-02:
`npm run test --prefix frontend -- BrowseView ClusterViewCustom NsViewCustom NsViewPods NsViewWorkloads queryBackedLeafFirstLoad useBrowseCatalog`
passed with 8 frontend test files and 130 frontend tests, covering catalog-backed
Browse, catalog-backed Custom resources, all-namespaces Pods/Workloads, and the
Phase 3 migrated table leaves. `npm run typecheck --prefix frontend` also
passed.
...
- The exact all-namespaces first-load class of bug is covered for query-backed
  Browse, Custom resources, Pods, Workloads, namespace tables, and cluster
  tables.
