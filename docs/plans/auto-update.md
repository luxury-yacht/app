...
- [ ] **Stage 3 — Linux:** publish the explicitly identified user-owned portable
  ...
  - [x] Linux-native tests prove portable install, interrupted-install
    ...
    guarded install/uninstall cleanup; the repeatable `linux:drill:packages`
    container drill proves both dpkg and rpm install and remove their distinct
    marker while runtime contract tests keep them notification-only.
