...
| Namespace Helm | `frontend/src/modules/namespace/components/NsViewHelm.tsx` | `backend/refresh/snapshot/namespace_helm.go` | namespace / all-namespaces | single namespace queries Helm directly; all-namespaces is capped by `SnapshotNamespaceHelmEntryLimit` | local sort/search/namespace over the loaded window | Local Partial for all-namespaces while capped; Local Complete for bounded single namespace | visible/capped-window only until query-backed |
