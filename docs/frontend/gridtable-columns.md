...
[metadata contract](gridtable-metadata.md).

## Read by change

- Definitions, durable keys, factories and absent values: [definitions](#column-definitions).
- Visibility, order, drag/drop and reset: [Columns menu](#columns-menu).
- Widths, measurement, resizing or virtualization measurement: [sizing](#column-sizing).
- Sort semantics: [sorting](#sorting).
- Age or utilization: [age and metrics](#age-and-metrics-columns).
