...
| Refresh execution            | `frontend/src/core/refresh/orchestrator.ts`, `frontend/src/core/refresh/RefreshManager.ts`                         | Orchestrator fetches one cluster scope at a time through per-cluster runtimes; manager schedules. Coordinator owns lifecycle, runtimes own cluster data. |
