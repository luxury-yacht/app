...
| Resource streams             | `backend/refresh/resourcestream`                                                                                                                                             | Stream rows must match snapshot row shape; WebSocket resource scopes are single-cluster.    |
...
| Refresh execution            | `frontend/src/core/refresh/orchestrator.ts`, `frontend/src/core/refresh/RefreshManager.ts`                         | Orchestrator fetches one cluster scope at a time through per-cluster runtimes; manager schedules. |
| Resource stream descriptors  | `frontend/src/core/refresh/streaming/resourceStreamDomains.ts`                                                      | Owns resource stream scope kind, row identity, sorting, drift keys, and row access. |
