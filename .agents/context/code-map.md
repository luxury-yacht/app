...
| Stream route wiring          | `backend/refresh/system/streams.go`                                                                                                                                          | Resources/events/catalog share the resource WebSocket doorbell; logs keep their SSE stream. |
| Refresh HTTP API             | `backend/refresh/api/server.go`                                                                                                                                              | Private Wails webview transport. Preserve CORS for webview access, preflight, and readable errors; keep cluster scope and request validation explicit. |
| Object catalog               | `backend/objectcatalog`, `backend/objectcatalog/identity.go`, `backend/refresh/snapshot/catalog.go`                                                                          | Source of truth for discovery, GVK/GVR identity, browse, namespace metadata, and cluster listings. Namespace LIST rows come from the `namespaces` refresh domain. |
...
| Rich detail/actions          | `backend/resources/<kind>` (`details.go`, `actions.go`), `backend/object_detail_provider.go` (generated dispatch via `mise exec -- go generate ./backend`)                                | Detail and imperative operations only, not list/table refresh data. Typed detail fetchers require exact-GVK capability metadata. |
...
| Wails bindings               | `backend` exported app methods, `frontend/wailsjs/go/models.ts`                                                                                                              | `mise exec -- wails generate` may not be reliable in every local run; validate with frontend typecheck. |

## Frontend

| Area                         | Start Here                                                                                                         | Notes                                                                              |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| Refresh domain types         | `backend/internal/genrefreshcontracts/registry.go`, `backend/refresh/domain/refresh-domain-contract.json`, generated into `frontend/src/core/refresh/types.generated.ts` | Register backend-owned DTOs/enums in Go and each domain's `refreshPayloadType` in the shared contract, then run `mise exec -- go generate ./backend`; keep frontend-only reducer state in `types.ts`. |
...
| ---------------------------------- | --------------------------------------------------------------------------------------------- |
| Backend resource model or snapshot | Focused `mise exec -- go test` package(s), then broader `mise exec -- go test ./backend/...` if shared behavior changed |
| Frontend component/hook            | Targeted Vitest spec, `mise exec -- npm run typecheck --prefix frontend`                                   |
| Refresh domain                     | Backend snapshot/system tests, frontend refresh tests, diagnostics panel check                |
| Logs/shell/port-forward/drain      | Focused backend session/stream tests, `mise exec -- go test ./backend/refresh/snapshot`, and UI lifecycle/orchestrator tests when present |
...
| Broad frontend/shared cleanup      | Targeted tests, `mise exec -- npm run typecheck --prefix frontend`, `mise exec -- mage qc:knip`                         |
| Final non-doc completion           | `mise exec -- mage qc:prerelease`, then inspect `git status`                                               |
