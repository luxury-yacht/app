...
7. For rendered Wails UI work, read `.agents/setup/browser-automation.md`.
8. For non-documentation work, finish with the latest code validated by
   `mise exec -- mage qc:prerelease` unless the task explicitly does not allow it.

## Task Routing

| Request                                                                    | First skill/doc                                                                    | First code paths                                                                                          | Validation                                                                      |
| -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Large-scale structural review for simplification, hardening, optimization, or refactoring across broad systems or cross-cutting concerns | `.agents/skills/app-review/SKILL.md`, `.agents/context/app-areas.md`               | Cross-cutting inventories first, then owning docs/contracts and system entrypoints                         | Usually read-only; plan docs use `git diff --check`; implementation uses `mise exec -- mage qc:prerelease` |
| "Is this branch production-ready?" or "ready to merge?"                    | `.agents/skills/branch-review/SKILL.md`                                            | `git diff origin/main...HEAD`, changed contract paths                                                     | Targeted tests, `mise exec -- mage qc:prerelease`, `git diff --check`                        |
...
| "Add support for resource X"                                               | `.agents/skills/add-resource/SKILL.md`                                             | `backend/resourcemodel`, `backend/refresh/snapshot`, `backend/resources`, object-panel/frontend consumers | Targeted backend/frontend tests, `mise exec -- mage qc:prerelease`                           |
| Refresh, stream, snapshot, diagnostics, or domain changes                  | `.agents/skills/refresh-subsystem/SKILL.md`, `docs/architecture/data-freshness.md`, `docs/architecture/refresh-system.md` | `backend/refresh/system`, `backend/refresh/snapshot`, `frontend/src/core/refresh`                         | Domain-specific backend/frontend tests, diagnostics check, `mise exec -- mage qc:prerelease` |
| Resource utilization metrics                                               | `.agents/skills/refresh-subsystem/SKILL.md`, `.agents/skills/object-panel/SKILL.md`, `docs/architecture/resource-metrics.md` | `frontend/src/core/resource-metrics`, metric-bearing snapshot domains, object-panel utilization consumers | Resource-metrics Vitest tests, affected table/panel tests, typecheck, `mise exec -- mage qc:prerelease` |
| Object identity, status, links, facts, lifecycle, or relationship behavior | `.agents/skills/shared-resource-model/SKILL.md`, `docs/architecture/shared-resource-model.md` | `backend/resourcemodel`, `backend/refresh/snapshot`, frontend status/link utilities                       | Parity tests across changed surfaces, `mise exec -- mage qc:prerelease`                      |
...
| Object panel details, YAML, actions, or docked panels                      | `.agents/skills/object-panel/SKILL.md`, `docs/frontend/dockable-panels.md`, `docs/frontend/yaml-editor.md` | `frontend/src/modules/object-panel`, `frontend/src/shared/components/yaml`, `backend/resources`, object detail/YAML/action backends | Targeted backend/frontend tests, typecheck, `mise exec -- mage qc:prerelease`                |
...
| Object map data, missing kinds, graph links, legend, or G6 behavior        | `.agents/skills/object-map/SKILL.md`, `docs/workflows/object-map.md`               | `backend/refresh/snapshot/object_map.go`, `frontend/src/modules/object-map`, object-panel support         | Object-map backend tests, targeted Vitest, `mise exec -- mage qc:prerelease`                 |
...
| Live object age text                                                       | `docs/frontend/live-age.md`, `.agents/skills/browse-tables/SKILL.md`, `.agents/skills/object-panel/SKILL.md`, `.agents/skills/object-map/SKILL.md` | `frontend/src/shared/components/LiveAgeText.tsx`, `frontend/src/shared/hooks/useAgeClock.ts`, age-bearing table/panel/map consumers | Fake-timer Vitest tests, typecheck, `mise exec -- mage qc:prerelease` for non-doc changes    |
| New Storybook story                                                        | `.agents/skills/new-story/SKILL.md`                                                | Component plus all hook/provider dependencies                                                             | `mise exec -- npm run typecheck --prefix frontend` or equivalent                             |
...
- Resource utilization metrics use `frontend/src/core/resource-metrics`; object
  details are fallback only except the documented ReplicaSet exception.
- Object age is frontend-rendered from absolute timestamps; do not use snapshot
  refetches to advance relative age text.
- Use the object catalog for identity, discovery, Browse, catalog listings, and
  cluster listings. Namespace LIST data comes only from the `namespaces` refresh
  domain; do not infer it from catalog entries.
...
- After `mise exec -- mage qc:prerelease`, inspect the worktree because `lint:fix` can
  modify files.
